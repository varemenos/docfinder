$(function(){footerHeight=$("footer").height();$("#results").css("margin-bottom",(2*footerHeight));current=1;fancyWidth=parseInt(($(window).width())*0.95);
fancyHeight=parseInt(($(window).height())*0.95);$(".resultLink").fancybox({width:fancyWidth,height:fancyHeight,type:"iframe"});getQ=a("q");getLang=a("lang");
if(typeof(getLang)!="undefined"&&getLang!==null&&getLang!==""){if($('#lang option[name="'+getLang+'"]').html()!=null){$('#lang option[name="'+getLang+'"]').attr("selected",true);
...
if((d.total%10)>0){pages++;}}else{pages=1;}$("#results").hide();$("#info h3").html('Found "'+d.total+'" results.');$("#results .result").remove();for(var c=0;
c<d.results.length;c++){$("#results").append(' 							<div class="result">								<h3><a class="resultLink" href="'+d.results[c].url+'">'+d.results[c].name+"</a> <sup>["+d.results[c].type+']</sup></h3>								<p class="description">'+d.results[c].description+'</p>								<p class="synopsis">'+d.results[c].synopsis+'</p>								<div class="namespace">'+d.results[c].namespace+"</div>							</div>						");
